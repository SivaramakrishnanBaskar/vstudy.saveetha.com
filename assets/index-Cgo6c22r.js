    ...
    return o => {
        ...
        if (t) return () => {
            for (let i = 0; i < r.length; i++) {
                const n = r[i];
                typeof n == "function" ? n() : L(e[i], null)
            }
        }
    }
}
...
const V = ({
    ...
    let c = !0,
        a = s.useMemo(() => (c = !1, {
            ...
            register: d => (f.set(d, !1), () => f.delete(d))
        }), [t, f, r]);
    return n && c && (a = { ...a
    }), s.useMemo(() => {
...
const J = ({
    ...
    const [f, g] = D(h), c = s.useMemo(() => k(e), [e]), a = h && !f ? [] : c.map(M), d = s.useRef(!0), m = s.useRef(c), l = z(() => new Map), [P, I] = s.useState(c), [C, v] = s.useState(c);
    H(() => {
    ...
    if (c !== P) {
        ...
        for (let u = 0; u < C.length; u++) {
            const R = C[u],
                j = M(R);
            a.includes(j) || (p.splice(u, 0, R), w.push(R))
        }
        return n === "wait" && w.length && (p = w), v(k(p)), I(c), null
    }
    const {
        forceRender: b
    } = s.useContext(K);
    return y.jsx(y.Fragment, {
        children: C.map(p => {
            const u = M(p),
                ...
                j = () => {
                    if (l.has(u)) l.set(u, !0);
                    else return;
                    let $ = !0;
                    l.forEach(S => {
                        S || ($ = !1)
                    }), $ && (b ? .(), v(m.current), h && g ? .(), r && r())
                };
            return y.jsx(V, {
                isPresent: R,
