var R = {};
(function V(M, T, B, I) {
        var k = !!(M.Worker && M.Blob && M.Promise && M.OffscreenCanvas && M.OffscreenCanvasRenderingContext2D && M.HTMLCanvasElement && M.HTMLCanvasElement.prototype.transferControlToOffscreen && M.URL && M.URL.createObjectURL),
        ...
        var x = (function(e, r) {
            ...
            _ = (function() {
                ...
                function t(o) {
                    ...
                    o.init = function(n) {
                        ...
                        return r = P(function(u) {
                            ...
                            o.addEventListener("message", f), i(n, d), a[d] = f.bind(null, {
                                data: {
                                    callback: d
                                }
                            })
                ...
                return function() {
                    if (e) return e;
                    if (!B && k) {
                        var o = ["var CONFETTI, SIZE = {}, module = {};", "(" + V.toString() + ")(this, module, true, SIZE);", "onmessage = function(msg) {", "  if (msg.data.options) {", "    CONFETTI(msg.data.options).then(function () {", "      if (msg.data.callback) {", "        postMessage({ callback: msg.data.callback });", "      }", "    });", "  } else if (msg.data.reset) {", "    CONFETTI && CONFETTI.reset();", "  } else if (msg.data.resize) {", "    SIZE.width = msg.data.resize.width;", "    SIZE.height = msg.data.resize.height;", "  } else if (msg.data.canvas) {", "    SIZE.width = msg.data.canvas.width;", "    SIZE.height = msg.data.canvas.height;", "    CONFETTI = module.exports.create(msg.data.canvas);", "  }", "}"].join(`
`);
                        try {
        ...
        var n = Math.PI / 10 * r.wobble,
            ...
            u = r.shape.bitmap.height * r.scalar,
            f = new DOMMatrix([Math.cos(n) * s, Math.sin(n) * s, -Math.sin(n) * h, Math.cos(n) * h, r.x, r.y]);
        f.multiplySelf(new DOMMatrix(r.shape.matrix));
        var m = e.createPattern(x.transform(r.shape.bitmap), "no-repeat");
        m.setTransform(f), e.globalAlpha = 1 - a, e.fillStyle = m, e.fillRect(r.x - d / 2, r.y - u / 2, d, u), e.globalAlpha = 1
    } else if (r.shape === "circle") e.ellipse ? e.ellipse(r.x, r.y, Math.abs(i - t) * r.ovalScalar, Math.abs(l - o) * r.ovalScalar, Math.PI / 10 * r.wobble, 0, 2 * Math.PI) : ar(e, r.x, r.y, Math.abs(i - t) * r.ovalScalar, Math.abs(l - o) * r.ovalScalar, Math.PI / 10 * r.wobble, 0, 2 * Math.PI);
    else if (r.shape === "star")
        for (var c = Math.PI / 2 * 3, g = 4 * r.scalar, p = 8 * r.scalar, y = r.x, w = r.y, C = 5, b = Math.PI / C; C--;) y = r.x + Math.cos(c) * p, w = r.y + Math.sin(c) * p, e.lineTo(y, w), c += b, y = r.x + Math.cos(c) * g, w = r.y + Math.sin(c) * g, e.lineTo(y, w), c += b;
    else e.moveTo(Math.floor(r.x), Math.floor(r.y)), e.lineTo(Math.floor(r.wobbleX), Math.floor(o)), e.lineTo(Math.floor(i), Math.floor(l)), e.lineTo(Math.floor(t), Math.floor(r.wobbleY));
...
function N(e, r) {
    var a = !e,
        ...
        n = l ? _() : null,
        s = a ? Y : rr,
        h = e && n ? !!e.__confetti_initialized : !1,
        d = typeof matchMedia == "function" && matchMedia("(prefers-reduced-motion)").matches,
        u;

    function f(c, g, p) {
        for (var y = v(c, "particleCount", G), w = v(c, "angle", Number), C = v(c, "spread", Number), b = v(c, "startVelocity", Number), cr = v(c, "decay", Number), hr = v(c, "gravity", Number), ur = v(c, "drift", Number), j = v(c, "colors", K), dr = v(c, "ticks", Number), D = v(c, "shapes"), fr = v(c, "scalar"), vr = !!v(c, "flat"), U = X(c), z = y, O = [], mr = e.width * U.x, gr = e.height * U.y; z--;) O.push(nr({
            x: mr,
            y: gr,
            angle: w,
            spread: C,
...
function ir(e, r, a, t, o, i, l) {
    ...
    var h = new Path2D;
    return h.addPath(s, new DOMMatrix([Math.cos(l) * o, Math.sin(l) * o, -Math.sin(l) * i, Math.cos(l) * i, a, t])), h
}
...
function lr(e) {
    ...
    if (!a) {
        for (var l = 1e3, n = l, s = l, h = 0, d = 0, u, f, m = 0; m < l; m += 2)
            for (var c = 0; c < l; c += 2) i.isPointInPath(t, m, c, "nonzero") && (n = Math.min(n, m), s = Math.min(s, c), h = Math.max(h, m), d = Math.max(d, c));
        u = h - n, f = d - s;
        var g = 10,
            p = Math.min(g / u, g / f);
        a = [p, 0, 0, p, -Math.round(u / 2 + n) * p, -Math.round(f / 2 + s) * p]
    }
    return {
        type: "path",
        path: r,
...
function sr(e) {
    ...
    var i = 10 * a,
        ...
        s = n.getContext("2d");
    s.font = l;
    var h = s.measureText(r),
        d = Math.ceil(h.actualBoundingBoxRight + h.actualBoundingBoxLeft),
        u = Math.ceil(h.actualBoundingBoxAscent + h.actualBoundingBoxDescent),
        f = 2,
        m = h.actualBoundingBoxLeft + f,
